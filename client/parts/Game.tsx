...
/**
 * Main Sea Battle game component (with custom HTML5 DnD).
...
export default function Game() {
  ...
  // New: Ref for drag preview (optional, for better UX)
  ...
  /**
   * Custom drag start handler: Sets ship data on dataTransfer.
   * Creates a drag preview (ghost ship) for visual feedback.
   */
  const handleDragStart = (
    e: React.DragEvent<HTMLDivElement>,
    item: DragItem
  ) => {
    // Serialize item to JSON for drop
    e.dataTransfer.setData("application/json", JSON.stringify(item));
    e.dataTransfer.effectAllowed = "move";

    // Optional: Create custom drag preview (ghost ship)
  ...
  /**
   * Custom drag over handler: Allows drop and previews placement (e.g., highlight cells).
   * You'd expand this for full preview logic if needed (e.g., temporarily style ship cells).
   */
  const handleDragOver = (e: React.DragEvent<HTMLDivElement>) => {
    e.preventDefault(); // Required to allow drop
    e.dataTransfer.dropEffect = "move";
    // Optional: Add hover highlight logic here (e.g., via state or CSS)
  };

  /**
   * Custom drop handler: Extracts data, calls your existing handleShipDrop.
   */
  const handleDrop = (
    e: React.DragEvent<HTMLDivElement>,
    row: number,
  ...
  const DraggableShip = ({ ship }: { ship: Ship }) => (
    <div
      ...
      className={`bg-gray-800 flex items-center justify-center mb-1 cursor-move`}
      style={{ width: `${ship.length * 32}px`, height: "30px" }}
      ref={dragPreviewRef} // For ghost preview
    >
      {ship.name}
    </div>
  );
  ...
  const DraggableShipForMove = ({
    ...
    <div
      ...
      className={`!bg-red-700 text-white px-2 py-1 text-xs font-medium rounded flex items-center justify-center mb-1 cursor-move`}
      style={{ width: `${placement.ship.length * 32}px` }}
      ref={dragPreviewRef}
    >
      {placement.ship.name} {placement.orientation === "horizontal" ? "↔" : "↕"}
    </div>
  );
